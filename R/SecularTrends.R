createAndGetTrendsByTargetEvent <- function(connection,
  ...
  eventTimeWindowTempTableSql <- getEventWindowsTempTableSql(connection,
                                                             eventTimeWindows,
                                                             tempEmulationSchema)
  targetEventXrefTempTableSql <- getTargetEventXrefTempTableSql(connection,
                                                                targetEventXref,
                                                                tempEmulationSchema)
...
getTargetEventXrefTempTableSql <- function(connection, targetEventXref, tempEmulationSchema) {
  ...
  for (i in 1:nrow(targetEventXref)) {
    stmt <- paste0("SELECT ",
                   targetEventXref$targetCohortId[i],
                   " target_cohort_id, ",
                   targetEventXref$eventCohortId[i],
                   " event_cohort_id")
...
getEventWindowsTempTableSql <- function(connection, eventWindows, tempEmulationSchema) {
  ...
  for (i in 1:nrow(eventWindows)) {
    stmt <- paste0("SELECT ",
                   eventWindows$windowId[i],
                   " window_id, ",
                   eventWindows$windowStart[i],
                   " window_start, ",
                   eventWindows$windowEnd[i],
                   " window_end, ",
                   "'",
                   eventWindows$windowType[i],
                   "' window_type")
    unions <- paste(unions, stmt, sep = "\n")
