...
createCpdSegmented <- function(linearModel, Samples, Time, family, initcp) {
  ...
  o.seg.fix <- segmented::segmented(o,
                                    seg.Z = ~z,
                                    psi = initcp,
                                    npsi = 1,
...
cpdSave <- function(Samples, Time, o.seg, list4, family) {
  ...
  if (est_yes) {
    ...
    # then plot plotCpdSegmented(Time, Samples, o.seg, family) and save plot p <- recordPlot()
  }
  # save results
