...
plotSegmented <- function(tsData,
  ...
  fittedData <- tibble(fitted.value  = fittedData$fit,
  ...
  if (is.null(model$nameUV)) {
    ...
    for (i in 1:nrow(model$psi)) {
      ...
      dfPrespecifiedChangepoints <- rbind(dfPrespecifiedChangepoints,
                                          data.frame(x = psi.date,
                                                     y = psiY))
      ...
      if (i == 1) {
        ...
        dfLineSegment <- rbind(dfLineSegment, 
                               data.frame(xmin = xMinLine1, 
                                          xend = xEndLine1, 
                                          ymin = yMinLine1,
                                          yend = yEndLine1,
  ...
  if (nrow(model$psi) > 0) {
    ...
    for (i in 1:nrow(model$psi)) {
      ...
      if (!is.null(term)) {
        ...
        dfEstimatedChangepoints <- rbind(dfEstimatedChangepoints,
                                         data.frame(x = psi.date,
                                                    y = psiY,
  ...
  if (nrow(dfEstimatedChangepoints) > 0) {
    p <- p +
      ...
      ggplot2::geom_errorbar(ggplot2::aes(xmin = ci_lb,
                                          xmax = ci_ub,
                                          x = x,
                                          y = y,
                                          color = "Est. Changepoint"), data = dfEstimatedChangepoints)
  }
